...
const Masonry = ({
  ...
  const grid = useMemo(() => {
    ...
    const targetHeight = containerHeight || window.innerHeight;

    const colHeights = new Array(columns).fill(0);
    const gridItems = [];
    
    let itemIndex = 0;
    while (Math.min(...colHeights) < targetHeight && gridItems.length < 200 && items.length > 0) {
      const child = items[itemIndex % items.length];
      ...
      gridItems.push({
        ...child,
        id: `${child.id}-rep-${gridItems.length}`,
        x,
        y,
        w: columnWidth,
        h: itemHeight
  ...
  useLayoutEffect(() => {
    ...
    grid.forEach((item, index) => {
      ...
      if (!hasMounted.current) {
        ...
        gsap.fromTo(selector(itemSelector), initialState, {
          opacity: 1,
          ...animationProps,
