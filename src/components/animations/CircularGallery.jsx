...
class Media {
  ...
  update(scroll, direction) {
    ...
    const relX = this.plane.position.x;
    if (relX < -this.widthTotal / 2) {
      this.extra -= this.widthTotal;
      this.plane.position.x += this.widthTotal;
    } else if (relX > this.widthTotal / 2) {
      this.extra += this.widthTotal;
      this.plane.position.x -= this.widthTotal;
    }
...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    while (repeatedItems.length < 18) {
      repeatedItems = repeatedItems.concat(galleryItems);
    }
    this.mediasImages = repeatedItems;
  ...
  onTouchMove(e) {
    if (!this.interactive || !this.isDown) return;
...
export default function CircularGallery({
  ...
  return (
    <div
      className={`circular-gallery${interactive ? ' interactive' : ''}`}
      ref={containerRef}
      tabIndex={interactive ? 0 : -1}
      role={interactive ? "region" : "presentation"}
      aria-label={interactive ? "Circular image gallery. Use left and right arrow keys to navigate." : undefined}
    />
  );
}
