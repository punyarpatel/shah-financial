...
const ScrollStack = ({
  ...
  const measureOffsets = useCallback(() => {
    ...
    // Temporarily clear transforms to measure static/untransformed positions
    const savedStyles = cardsRef.current.map(card => {
      if (!card) return null;
      return {
        transform: card.style.transform,
        webkitTransform: card.style.webkitTransform,
    ...
    if (endElement) {
      if (useWindowScroll) {
        ...
        endOffsetRef.current = rect.top + scrollTop;
      } else {
    ...
    cardsRef.current.forEach((card, i) => {
      if (card && savedStyles[i]) {
        card.style.transform = savedStyles[i].transform;
        card.style.webkitTransform = savedStyles[i].webkitTransform;
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    if (!cardOffsetsRef.current || cardOffsetsRef.current.length === 0) {
      measureOffsets();
    }
    ...
    // Calculate the top card index for blur transitions once outside the loop
    let topCardIndex = 0;
    if (blurAmount) {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const cardHeight = card.clientHeight || 500;
      let pinEnd = containerBottom - stackPositionPx - itemStackDistance * i - cardHeight;
      pinEnd = Math.max(pinStart, Math.min(pinEnd, maxPinEnd));
